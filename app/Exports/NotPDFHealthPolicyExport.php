...
class NotPDFHealthPolicyExport implements FromQuery, WithHeadings, ShouldAutoSize, WithEvents, WithMapping
{
    ...
    {
        $request = [];
        $getPolicy = HealthPolicy::query()->where('policy_copy_status', 1)->where('status', 1)->with(['product', 'sub_product', 'customer', 'branch', 'company', 'company_branch', 'branch_imd_name', 'payments']);
        if ($this->agent != '') {
