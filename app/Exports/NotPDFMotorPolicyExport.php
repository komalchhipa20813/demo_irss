...
class NotPDFMotorPolicyExport implements FromQuery, WithHeadings, ShouldAutoSize, WithEvents, WithMapping
{
    ...
    {
        $request = [];
        $motorPolicy = MotorPolicy::query()->where('policy_copy_status', 1)->where('status', 1)->with(['product', 'sub_product', 'customer', 'branch', 'company', 'companyBranch', 'branch_imd_name', 'payments', 'motor_policy_vehicle']);
        if ($this->agent != '') {
