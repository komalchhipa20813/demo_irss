...
class ExportUpdateHealthPolicy implements FromQuery, WithHeadings, ShouldAutoSize, WithEvents, WithMapping
{
    ...
    */
    ...
    {
        $request=[];
        $getPolicy =  HealthPolicy::query()->where('status','=',1)->where('policy_number',null)->with('customer','agent','previous_company','renewal_previous_company','company');
        if($this->agent != ''){
