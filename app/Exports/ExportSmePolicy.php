...
class ExportSmePolicy implements FromQuery, WithHeadings, ShouldAutoSize, WithEvents, WithMapping
{
    ...
    */
    ...
    {
        $request=[];
        $getPolicy = SmePolicy::query()->where('status', 1)->with(['sub_product','product','company','customer','agent']);
        if($this->agent != ''){
