...
class NotPDFSMEPolicyExport implements FromQuery, WithHeadings, ShouldAutoSize, WithEvents, WithMapping
{
    ...
    {
        $request = [];
        $getPolicy = SmePolicy::where('status', '!=', 2)->where('policy_copy_status', 1)->with(['product', 'sub_product', 'customer', 'branch', 'company', 'companyBranch', 'branch_imd_name', 'payments']);
