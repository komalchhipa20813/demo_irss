...
class ExportMotorPolicy implements FromQuery, WithHeadings, ShouldAutoSize, WithEvents, WithMapping
{
    ...
    */
    ...
    {
        $request=[];
        $motorPolicy = MotorPolicy::query()->where('status', 1)->with(['sub_product','product','company','customer','agent']);
            if($this->agent != ''){
