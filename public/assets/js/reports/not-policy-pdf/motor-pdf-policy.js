$(document).ready(function(){
    ...
    if($("#not-uploaded-policy_tbl").length){
        function policy_listing(filterdata){
            $("#not-uploaded-policy_tbl").DataTable({
                processing: true,
                serverSide: true,
                aLengthMenu: [
                    [10, 30, 50, -1],
                ...
                ajax: {
                    type: "POST",
                    data:filterdata,
                    url: aurl + "/not-uploaded-policy/motor",
                },
                columns: [
                    { data: '0' },
                    { data: '1' },
    ...
    $('body').on('click','#filter_motor_policy',function(){
        var data = {
            ...
            $('#not-uploaded-policy_tbl').DataTable().destroy();
            policy_listing(data);
    });
    ...
    $("#export").click(function(event){
        ...
        $.ajax({
                url: aurl + "/not-uploaded-policy/export-motor-data",
                type: "POST",
                data: {
                        data:data,
                      },
